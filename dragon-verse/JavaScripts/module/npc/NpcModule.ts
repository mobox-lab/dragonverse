...
export class NpcModuleC extends ModuleC<NpcModuleS, NpcModuleData> {
    ...
    public static async NpcPrefabFactory(
        config: INpcElement) {
        if (!config) {
            Log4Ts.error(NpcModuleC, `invalid config when generate npc.`);
        }
        if (!config.position) {
            Log4Ts.error(NpcModuleC, `generate item param location invalid.`);
            return null;
        }
    ...
    private initAllNpc() {
        GameConfig
            ...
            .forEach(
                config => {
                    Log4Ts.log(NpcModuleC, `try generate npc. ${config.id}`);
...
export class NpcModuleS extends ModuleS<NpcModuleC, NpcModuleData> {
    //#region Member
    private _eventListeners: EventListener[] = [];
    //#endregion ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠐⠒⠒⠒⠒⠚⠛⣿⡟⠄⠄⢠⠄⠄⠄⡄⠄⠄⣠⡶⠶⣶⠶⠶⠂⣠⣶⣶⠂⠄⣸⡿⠄⠄⢀⣿⠇⠄⣰⡿⣠⡾⠋⠄⣼⡟⠄⣠⡾⠋⣾⠏⠄⢰⣿⠁⠄⠄⣾⡏⠄⠠⠿⠿⠋⠠⠶⠶⠿⠶⠾⠋⠄⠽⠟⠄⠄⠄⠃⠄⠄⣼⣿⣤⡤⠤⠤⠤⠤⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄
