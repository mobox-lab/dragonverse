...
class HyperText {
    ...
    static fromString(str, tag, attr) {
        ...
        while (walked < str.length) {
            ...
            if (result) {
                ...
                hyperText.content.push(HyperText.fromString(str.slice(pureLeft + result[1].length + (result[2]?.length ?? 0) + 2, pureLeft + result[1].length + (result[2]?.length ?? 0) + 2 + result[3].length), result[1], result[2]));
...
HyperText.RegTag = /<\s*(u|b|i|s|color|size)\s*(=.*?)?>(.*?)<\s*\/\1\s*>/g;
/**
...
export { Delegate, GtkTypes, HyperText, ObjectPool, RandomGenerator, Regulator, Singleton, Switcher, Gtk as default };
