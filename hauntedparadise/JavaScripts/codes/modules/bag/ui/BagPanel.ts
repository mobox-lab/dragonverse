...
export default class BagPanel extends Bag_UI_Generate {
    ...
    private onItemAdd(itemData: BagItemData, needEquip: boolean) {
        ...
        if (bagItemUI) {
            ...
            // 在可视范围之内是可装备的才装备上
            if (needEquip && itemData.nodeId < BagDefine.ViewCount && this.curView === 1) { EquipDefine.EquipItem(itemData.guid); }
        }
    }
    ...
    private onItemRemove(itemData: BagItemData) {
        ...
        if (!itemUI) { itemUI = this.normalItemContainer.nodes.find(v => { return v.data && v.data.guid === itemData.guid }); }
    ...
    private getItemUIByNodeId(nodeId: number) {
        ...
        } else if (nodeId < BagDefine.ViewCount + BagDefine.NormalItemCapacity) {
    ...
    private getFirstEmptyGridIdFromContainer(container: GridSelectContainer<BagItemUI>) {
        return container.nodes.findIndex((itemUI) => { return itemUI.data === null });
    }
    ...
    public setAllNodeEmpty() {
        ...
        this.normalItemContainer.nodes.forEach(itemUI => { itemUI.setAsEmpty(); });
        this.specialItemContainer.nodes.forEach(itemUI => { itemUI.setAsEmpty(); });
    }
    ...
    public reqChangeTwoItem(itemUI1: BagItemUI, itemUI2: BagItemUI) {
        if (!this.canChangeTwoItem(itemUI1, itemUI2)) { Tips.show(GameConfig.SubLanguage.bag_01.Value); return; }
    ...
    private canChangeTwoItem(itemUI1: BagItemUI, itemUI2: BagItemUI) {
        if (this.isSpecialItem(itemUI1.data.cfgId) && itemUI2.type === EBagItemType.Normal) { return false; }
        if (!this.isSpecialItem(itemUI1.data.cfgId) && itemUI2.type === EBagItemType.Special) { return false; }
        if (itemUI2.data && this.isSpecialItem(itemUI2.data.cfgId) && itemUI1.type === EBagItemType.Normal) { return false; }
        if (itemUI2.data && !this.isSpecialItem(itemUI2.data.cfgId) && itemUI1.type === EBagItemType.Special) { return false; }
        return true;
    }
