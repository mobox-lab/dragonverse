...
class GToolkit {
    constructor() {
        ...
        this._batchHandlerPool = new Map();
        this._repeatBatchHandlerPool = new Map();
         ...
         * 最小 UI 安全透明度.
         */
        this.MIN_SAFE_UI_RENDER_OPACITY = 1 / 255;
        /**
         * 该平台是否 use mouse 缓存.
         * @type {boolean}
         * @private
     ...
     */
    batchDo(data, batchCallback, waitTime = undefined, reTouch = false, instantly = false) {
        let existBatch = this.tryGet(this._batchHandlerPool, batchCallback, () => ({
            timerId: undefined,
            data: [],
            delayDo: () => {
        ...
        if (instantly) {
            ...
            if (waitTime !== undefined)
                existBatch.lastWaitDuration = waitTime;
            existBatch.timerId = mw.setTimeout(existBatch.delayDo, existBatch.lastWaitDuration ?? 0.1e3);
        }
     ...
     * @return {number} timer id.
     */
    batchRepeatDo(data, batchCallback, waitTime = undefined, reTouch = false, instantly = false) {
        let existBatch = this.tryGet(this._repeatBatchHandlerPool, batchCallback, () => ({
            timerId: undefined,
            data: [],
            delayDo: () => {
                if (existBatch.timerId !== undefined) {
    ...
    waitDo(data, waitCallback, waitTime = undefined, reTouch = true, instantly = false) {
        let existBatch = this.tryGet(this._waitHandlerPool, waitCallback, () => ({
            timerId: undefined,
            data: undefined,
            delayDo: () => {
                if (existBatch.timerId !== undefined) {
        ...
        if (instantly) {
            ...
            if (waitTime !== undefined)
                existBatch.lastWaitDuration = waitTime;
            existBatch.timerId = mw.setTimeout(existBatch.delayDo, existBatch.lastWaitDuration ?? 1e3);
        }
        return existBatch.timerId;
    }
    /**
     ...
     * @return {string[]}
     */
    getAllMember(obj, ignoreObjectProto = true, ignoreConstructor = true) {
        let props = [];
        let currentObj = obj;
        while (currentObj &&
            (!ignoreObjectProto || currentObj !== Object.prototype)) {
            props.push(...(ignoreConstructor ?
     ...
     */
    findPropertyDescriptor(obj, prop) {
        while (obj != null) {
     ...
     */
    manhattanDistance(a, b = undefined) {
        let result = 0;
     ...
     * @param b
     */
    squaredEuclideanDistance(a, b = undefined) {
        let result = 0;
        if (a instanceof Array) {
            if (b && a.length !== b.length)
                return result;
     ...
     * @param b
     */
    euclideanDistance(a, b = undefined) {
     ...
     * 当 b 为 undefined 时 将 a 视为向量. 并计算其长度平方.
     */
    manhattanDistance2(a, b = undefined) {
        let result = 0;
        result = Math.abs(a.x - (b ? b.x : 0)) +
            Math.abs(a.y - (b ? b.y : 0));
        return result;
     ...
     * @param b
     */
    squaredEuclideanDistance2(a, b = undefined) {
        let result = 0;
        result = Math.pow(a.x - (b ? b.x : 0), 2) +
            Math.pow(a.y - (b ? b.y : 0), 2);
        return result;
     ...
     * @param b
     */
    euclideanDistance2(a, b = undefined) {
     ...
     * 当 b 为 undefined 时 将 a 视为向量. 并计算其长度平方.
     */
    manhattanDistance3(a, b = undefined) {
        let result = 0;
        result = Math.abs(a.x - (b ? b.x : 0)) +
            Math.abs(a.y - (b ? b.y : 0)) +
     ...
     * @param b
     */
    squaredEuclideanDistance3(a, b = undefined) {
        let result = 0;
        result = Math.pow(a.x - (b ? b.x : 0), 2) +
            Math.pow(a.y - (b ? b.y : 0), 2) +
     ...
     * @param b
     */
    euclideanDistance3(a, b = undefined) {
        return Math.sqrt(this.squaredEuclideanDistance(a, b));
    }
    /**
     ...
     *      - null 或 undefined 无限遍历.
     */
    getUiChildren(widget, traverse = 1) {
        if (!widget)
            return [];
        let result = [];
        for (let i = 0; i < widget.getChildrenCount(); ++i)
            result.push(widget.getChildAt(i));
        let p = 0;
        let traversed = 1;
        while (p < result.length && (this.isNullOrUndefined(traverse) || traversed < traverse)) {
            const currLength = result.length;
    ...
    whyUiNotShown(widget) {
        ...
        while (focus) {
            let reason = WidgetNotShownReason.Null;
    ...
    async updateModuleData(moduleDataName, userId, value) {
        const data = await mw.DataStorage.asyncSetData(this.getModuleDataKey(userId, moduleDataName), value);
 ...
 * @flag
 */
var WidgetNotShownReason;
(function (WidgetNotShownReason) {
    /**
     * 空置.
     * @type {WidgetNotShownReason.Null}
     */
    WidgetNotShownReason[WidgetNotShownReason["Null"] = 0] = "Null";
    /**
     * 透明的.
     * @desc render opacity 为 0.
     */
    WidgetNotShownReason[WidgetNotShownReason["Transparent"] = 1] = "Transparent";
    /**
     * 隐藏的.
     * @desc visibility = {@link mw.Visiblity.Hidden}.
     */
    WidgetNotShownReason[WidgetNotShownReason["Hidden"] = 2] = "Hidden";
    /**
     * 收缩的.
     * @desc visibility = {@link mw.Visibility.Collapsed}.
     */
    WidgetNotShownReason[WidgetNotShownReason["Collapsed"] = 4] = "Collapsed";
    /**
     * 越界的.
     * @desc 超出 Viewport.
     */
    WidgetNotShownReason[WidgetNotShownReason["OutOfView"] = 8] = "OutOfView";
    /**
     * 过小的.
     * @desc 面积小于 10 px.
     * @desc x * y * scale
     */
    WidgetNotShownReason[WidgetNotShownReason["TooSmall"] = 16] = "TooSmall";
    /**
     * 未绘制按钮.
     * @desc 作为 Button 时，未绘制图片.
     */
    WidgetNotShownReason[WidgetNotShownReason["BtnImageNoDraw"] = 32] = "BtnImageNoDraw";
    /**
     * 未挂载.
     * @desc 未挂载于 ViewPort.
     */
    WidgetNotShownReason[WidgetNotShownReason["NotAttached"] = 64] = "NotAttached";
})(WidgetNotShownReason || (WidgetNotShownReason = {}));
//#endregion ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠐⠒⠒⠒⠒⠚⠛⣿⡟⠄⠄⢠⠄⠄⠄⡄⠄⠄⣠⡶⠶⣶⠶⠶⠂⣠⣶⣶⠂⠄⣸⡿⠄⠄⢀⣿⠇⠄⣰⡿⣠⡾⠋⠄⣼⡟⠄⣠⡾⠋⣾⠏⠄⢰⣿⠁⠄⠄⣾⡏⠄⠠⠿⠿⠋⠠⠶⠶⠿⠶⠾⠋⠄⠽⠟⠄⠄⠄⠃⠄⠄⣼⣿⣤⡤⠤⠤⠤⠤⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄
//#region Delegate
/**
 * Delegate. 委托.
...
class RandomGenerator {
     ...
     * @return {this}
     */
    randomCircle(randomFunc = Math.random) {
...
class RevisedInterval {
     ...
     */
    constructor(_callback, _interval, delay = 0, instant = false) {
        this._callback = _callback;
        this._interval = _interval;
        this.revisedIntervalHandler = () => {
            const now = Date.now();
            this.safeCallback();
...
const singleFrameCacheHoldMap = new Map();
let isSingleFrameCacheRegistered = false;
/**
 * 单帧缓存.
 ...
 */
function SingleFrameCache(dirtyPred) {
...
//#endregion ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠐⠒⠒⠒⠒⠚⠛⣿⡟⠄⠄⢠⠄⠄⠄⡄⠄⠄⣠⡶⠶⣶⠶⠶⠂⣠⣶⣶⠂⠄⣸⡿⠄⠄⢀⣿⠇⠄⣰⡿⣠⡾⠋⠄⣼⡟⠄⣠⡾⠋⣾⠏⠄⢰⣿⠁⠄⠄⣾⡏⠄⠠⠿⠿⠋⠠⠶⠶⠿⠶⠾⠋⠄⠽⠟⠄⠄⠄⠃⠄⠄⣼⣿⣤⡤⠤⠤⠤⠤⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄
//#region MwTransform
class SimulatedWorldTransform {
    get position() {
        if (!this.parent)
            return this.localPos;
        if (!this._cached)
            this._cached = { x: 0, y: 0, z: 0 };
 ...
 */
class MwTransform {
    constructor(localPosition, parent) {
        const simulatedWorldTransform = new SimulatedWorldTransform(localPosition ?? { x: 0, y: 0, z: 0 }, parent);
 ...
 */
function createProvider(selector) {
    if (!selector || selector.length <= 0)
        return undefined;
    let criterion = selector[0];
    let provider = {};
    const providerHandler = function (funcName, args) {
    ...
    if (enumerable === Object.prototype)
...
export { Delegate, EditorVersion, GtkTypes, HyperText, LinkedLocalTransform, MwTransform, ObjectPool, RandomGenerator, Regulator, RevisedInterval, SimulatedWorldTransform, SingleFrameCache, Singleton, Switcher, WidgetNotShownReason, createProvider, Gtk as default };
