...
export class TalentTreeContainer extends TalentTreeContainer_Generate {
    ...
    private onTalentSelected(id: number) {
        ...
        // 初始化选择框 UI
        if (!this._selectedTalentUI) this._selectedTalentUI = UIService.create(UITalentItem);
        const selectedTalentUI = this._selectedTalentUI;
        selectedTalentUI.setData(talent.data);
        selectedTalentUI.canActive.status = talent.canActive.status;
        selectedTalentUI.setCurrentLevel(talent.level);
        this.talentItemCanvas.addChild(selectedTalentUI.uiObject);
    ...
    private updateLevelInfo(talent: TalentItem) {
        ...
        this._levelInfo.forEach((info, index) => {
            if (index < data.length) {
                ...
                if (level - 1 === index) info[1].setFontColorByHex("#FFCB1C");
    ...
    private onTalentLevelUpClick() {
        ...
        if (!item) return;
        this.talentC.tryTalentLevelUp(item.data.id);
    }
}