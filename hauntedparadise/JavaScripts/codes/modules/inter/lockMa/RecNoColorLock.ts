...
@Component
export default class RecNoColorLock extends LevelBase {
    // @Property({ displayName: "控制目标", capture: true })
    // public captrueTarget: string = "";

    @Property({ displayName: "正确角度范围" })
    public targetData: ReckLockData = new ReckLockData();
    ...
    private _isRever: boolean = false;

    protected onLevelStart(): void {
        if (SystemUtil.isServer()) {
            return;
        }
        this.addLocalListen("RecNoColorLock", (goId: string) => {
