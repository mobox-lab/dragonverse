...
import { Lui, Component, Button, Property, Dialogue, AutoComplete } from 'mw-lynx-ui';
...
import { ExpandIcon } from './icon/ExpandIcon.js';
import { MoveIcon } from './icon/MoveIcon.js';
import { PlatformIcon } from './icon/PlatformIcon.js';
import GodModGameConfigRenderer from './param-renderer/GodModGameConfigRenderer.js';
import GodModGameConfigParamInput from './param-input/GodModGameConfigParamInput.js';
import GodModRotationParamInput from './param-input/GodModRotationParamInput.js';
import GodModVector2ParamInput from './param-input/GodModVector2ParamInput.js';
...
class GodModPanel extends Component {
    constructor() {
        ...
        this._currentTouchBtnMoveLocation = undefined;
        ...
        this.renderAnimHandler = dt => {
            ...
            if (this._floating) {
                if (this._cnvMain.renderOpacity > 0) {
                    this._cnvMain.renderOpacity = Math.max(0, this._cnvMain.renderOpacity - dt / Interval.Fast);
                }
                if (this.root.position.x > -GodModPanelSizeX) {
                    ...
                    Gtk.setUiPositionX(this.root, Math.max(-GodModPanelSizeX, this.root.position.x - this._enterFloatDist * dt / Interval.Fast));
                }
                ...
                if (this._cnvMain.renderOpacity < 1) {
                    this._cnvMain.renderOpacity = Math.min(1, this._cnvMain.renderOpacity + dt / Interval.Fast);
                }
                if (this.root.position.x < 0) {
                    Gtk.setUiPositionX(this.root, Math.min(0, this.root.position.x + GodModPanelSizeX * dt / Interval.Fast));
                }
            }
            if (this._expandFloat) {
                if (this._cnvFloatWindow.position.x < GodModPanelSizeX) {
                    Gtk.setUiPositionX(this._cnvFloatWindow, Math.min(GodModPanelSizeX, this._cnvFloatWindow.position.x + GodModPanelSizeX * dt / Interval.Fast));
                }
                if (this._imgShrinkFloatBar.renderScale.x < 1) {
                    Gtk.setUiScaleX(this._imgShrinkFloatBar, Math.min(1, this._imgShrinkFloatBar.renderScale.x + dt / Interval.Fast));
                    this._imgExpandFloatBar.renderOpacity = Math.max(0, this._imgExpandFloatBar.renderOpacity - dt / Interval.Fast);
                }
            }
            else {
                if (this._cnvFloatWindow.position.x > GodModPanelSizeX - 200) {
                    Gtk.setUiPositionX(this._cnvFloatWindow, Math.max(GodModPanelSizeX - 200, this._cnvFloatWindow.position.x - GodModPanelSizeX * dt / Interval.Fast));
                }
                if (this._imgShrinkFloatBar.renderScale.x > 0) {
                    Gtk.setUiScaleX(this._imgShrinkFloatBar, Math.max(0, this._imgShrinkFloatBar.renderScale.x - dt / Interval.Fast));
                    this._imgExpandFloatBar.renderOpacity = Math.min(1, this._imgExpandFloatBar.renderOpacity + dt / Interval.Fast);
                }
            }
        };
        this.dragHandler = () => {
            if (this._dragStartTime === undefined ||
                Date.now() - this._dragStartTime < this._dragSensitive)
                return;
            const viewPortCanvas = mw.UIService.canvas;
            let currMouseAbsolutePos = this.btnMovePointerLocation;
            if (!currMouseAbsolutePos)
                return;
            if (this._mouseStartMosPos === undefined) {
                this._mouseStartMosPos = mw.absoluteToLocal(viewPortCanvas.cachedGeometry, this.btnMovePointerLocation);
                this._mouseStartCnvPos = this.root.position.clone();
                this.playStartDragEffect();
            }
            else {
                let currMouseRelativePos = mw.absoluteToLocal(viewPortCanvas.cachedGeometry, currMouseAbsolutePos);
    ...
    get btnMovePointerLocation() {
        if (Gtk.useMouse)
            return mw.getMousePositionOnPlatform();
        else
            return this._currentTouchBtnMoveLocation;
    }
    ...
    static create(option) {
        ...
        godModPanel._btnExpand.onClick.add(() => godModPanel.innerShowTips("打破常规的全新呼出方式. 敬请期待..."));
        ...
        godModPanel._btnMove["_btn"]["onTouchMoved"]
            .bind((pos, evt) => {
        ...
        }, undefined, undefined, godModPanel);
        ...
        godModPanel._cnvParamInputContainer = mw.Canvas.newObject(godModPanel._cnvMain, "cnvParamInputContainer");
        ...
        godModPanel._cnvParamInput = mw.Canvas.newObject(godModPanel._cnvParamInputContainer, "cnvParamInput");
        ...
        godModPanel._gameConfigRenderer = GodModGameConfigRenderer.create()
            .attach(godModPanel._cnvParamInput);
        Gtk.setUiPositionY(godModPanel._gameConfigRenderer.root, this.BtnRunSizeY +
            this.TxtInfoSizeY);
        ...
        godModPanel._imgExpandFloatBar = mw.Image.newObject(godModPanel._cnvFloatWindow, "imgExpandFloatBar");
        ...
        godModPanel._imgExpandFloatBar.setImageColorByHex(Lui.Asset.ColorUtil.colorHexWithAlpha(Lui.Asset.Color.Blue, 1));
        ...
        godModPanel._imgShrinkFloatBar = mw.Image.newObject(godModPanel._cnvFloatWindow, "imgShrinkFloatBar");
        ...
        godModPanel._imgShrinkFloatBar.setImageColorByHex(Lui.Asset.ColorUtil.colorHexWithAlpha(Lui.Asset.Color.Blue, 1));
    ...
    showCnvParamInput() {
        ...
        Gtk.setUiPositionY(this._gameConfigRenderer.root, paramSizeY +
            GodModPanel.BtnRunSizeY +
            GodModPanel.TxtInfoSizeY);
        this._currentInputComponent = input?.attach(this._cnvParamInput) ?? undefined;
        if (!this._currentChoose.isReadonlyCmd)
...
GodModPanel.AutoShrinkFloatTime = 3e3;
function typeIsConfig(type) {
    return Gtk.is(type, "getElement");
}
