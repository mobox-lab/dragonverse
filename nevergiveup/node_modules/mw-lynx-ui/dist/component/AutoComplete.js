...
import { Component, extractLayoutFromOption, overrideOption, getComponentFromRoot } from './Component.js';
import { TextField } from './TextField.js';
import { Lui } from '../style/Asset.js';
import Enumerable from 'linq';
import Fuse from 'fuse.js';
import { CommitType } from '../event/InputEvent.js';
...
class AutoComplete extends Component {
    constructor() {
        ...
        this.listenToUp = () => {
            ...
            if (index === this._standForIndexInView)
                return;
    ...
    clearRealCommitTimer() {
        if (this._realCommitTimer) {
            mw.clearTimeout(this._realCommitTimer);
            this._realCommitTimer = undefined;
        }
    }
    refreshRealCommitTimer(force) {
        if (this._realCommitTimer) {
            mw.clearTimeout(this._realCommitTimer);
        }
    ...
    standForByViewIndex(index) {
        if (index === undefined)
            index = this.findNonTagIndex(-1, 1);
    ...
    findNonTagIndex(start, step) {
        ...
        while (index >= 0 && index < maxCount) {
            ...
            if (!(getComponentFromRoot(ui)?.isTag ?? true)) {
                return index;
            }
            index += step;
        }
...
class AutoCompleteContentItem extends Component {
    constructor() {
        ...
        this.onPressStart = new SimpleDelegate();
        this.onPressEnd = new SimpleDelegate();
    ...
    setLayout(option) {
        ...
        if (!this._icon || this._option.iconAlign === "right") {
            ...
            Gtk.setUiPosition(this._txtItem, pl + this._icon.root.size.x + 5, pt);
            Gtk.setUiSize(this._txtItem, contentX - pl - pr - this._icon.root.size.x, contentY);
        }
        if (this._icon) {
            if (this._option.iconAlign === "left") {
                Gtk.setUiPosition(this._icon.root, pl + 30, pt + (contentY - this._icon.root.size.y) / 2);
            }
            else {
                Gtk.setUiPosition(this._icon.root, pl + contentX - this._icon.root.size.x - 30, pt + (contentY - this._icon.root.size.y) / 2);
            }
        }
        return this;
    }
