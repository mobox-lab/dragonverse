...
export default class CodeVerifyPanel extends CodePanel_Generate {
    //#region Member
    //#endregion ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠐⠒⠒⠒⠒⠚⠛⣿⡟⠄⠄⢠⠄⠄⠄⡄⠄⠄⣠⡶⠶⣶⠶⠶⠂⣠⣶⣶⠂⠄⣸⡿⠄⠄⢀⣿⠇⠄⣰⡿⣠⡾⠋⠄⣼⡟⠄⣠⡾⠋⣾⠏⠄⢰⣿⠁⠄⠄⣾⡏⠄⠠⠿⠿⠋⠠⠶⠶⠿⠶⠾⠋⠄⠽⠟⠄⠄⠄⠃⠄⠄⣼⣿⣤⡤⠤⠤⠤⠤⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄
    ...
    protected onAwake(): void {
        ...
        this.codeInputBox.onTextChanged.add((text) => {
            if (text.length < 6) this.codeButtonVerify.enable = false;
            if (text.length === 6) this.codeButtonVerify.enable = true;
            if (text.length > 6) this.codeInputBox.text = text.slice(0, 6);
        });

        InputUtil.onKeyDown(Keys.Enter, () => this.codeButtonVerify.onClicked.broadcast());
