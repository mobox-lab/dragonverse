    ...
    (player) => {
        const onFailed = (result: mw.TeleportResult) => {
            ...
            for (const userId in result.userIds) {
                ...
                if (player) {
                    mw.Event.dispatchToClient(player, "onJumpGameFailed", result.message);
                }
            }
        };
        mw.TeleportService.asyncTeleportToScene("pet-simulator", [player.userId]).then(() => {
        }, onFailed);
    },
);
...
AddGMCommand(
    ...
    (player) => {
        ...
        mw.TeleportService.asyncTeleportToScene("battleworld", [player.userId]).then(() => {
        }, onFailed);
    },
);
